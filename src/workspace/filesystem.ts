...
export function enforceSafePath(root: string, targetPath: string): string {
  ...
  if (resolved !== absoluteRoot && !resolved.startsWith(absoluteRoot + path.sep)) {
    throw new SecurityError(`Path traversal detected: Access denied to path outside workspace boundaries (${targetPath})`);
  }
  return resolved;
}
...
const DEFAULT_EXCLUDES = [
  ...
  '.output', '__pycache__', '.pytest_cache', 'target',
  ...
  async function walk(currentDir: string, depth: number) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        const isSourceExt = SOURCE_EXTENSIONS.includes(ext) || SOURCE_EXTENSIONS.includes(entry.name);
        if (isSourceExt) {
