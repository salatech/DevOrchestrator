...
export class ContextSelector {
  ...
  private scoreFile(
    ...
    const webKeywords = ['api', 'route', 'endpoint', 'handler', 'controller', 'page', 'component', 'middleware'];
    if (webKeywords.some(wk => keywords.includes(wk)) && webKeywords.some(wk => lowerPath.includes(wk))) {
      score += 0.25;
      reasons.push('web-related file matches request');
    }
    
    // Database/model files get a bonus for data-related keywords
    const dataKeywords = ['database', 'model', 'schema', 'migration', 'seed', 'prisma', 'db', 'table'];
    if (dataKeywords.some(dk => keywords.includes(dk)) && dataKeywords.some(dk => lowerPath.includes(dk))) {
      score += 0.25;
      reasons.push('data-related file matches request');
    }
    
    // Auth files get a bonus for auth-related keywords
    const authKeywords = ['auth', 'login', 'session', 'jwt', 'oauth', 'token', 'password', 'signup', 'register'];
    if (authKeywords.some(ak => keywords.includes(ak)) && authKeywords.some(ak => lowerPath.includes(ak))) {
      score += 0.25;
      reasons.push('auth-related file matches request');
    }
    
    // Test file associated with a matched source file
    if (score > 0 && (lowerPath.includes('.test.') || lowerPath.includes('.spec.') || lowerPath.includes('__tests__'))) {
      score += 0.1;
  ...
  private extractKeywords(request: string): string[] {
    const stopWords = new Set([
      ...
      'change', 'modify', 'remove', 'delete', 'use', 'using', 'existing',
    ]);
    
    const words = request.toLowerCase().split(/[^a-z0-9]+/).filter(Boolean);
