...
export function discoverSkills(request: string, skills: Skill[]): SkillContext[] {
  const requestWords = new Set(request.toLowerCase().split(/[^a-z0-9]+/).filter(w => w.length > 2));
  
  const scoredSkills = skills.map(skill => {
    let score = 0;
    const skillWords = new Set(skill.keywords);
    const whenToUseWords = new Set(skill.whenToUse.toLowerCase().split(/[^a-z0-9]+/).filter(w => w.length > 2));
