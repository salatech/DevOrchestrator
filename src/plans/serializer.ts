...
export function serializePlan(plan: Plan): string {
  ...
  if (plan.relevantFiles?.length) result += `# Relevant Files\n${plan.relevantFiles.map(f => `- ${f}`).join('\n')}\n\n`;
  if (plan.filesToModify?.length) result += `# Files To Modify\n${plan.filesToModify.map(f => `- ${f}`).join('\n')}\n\n`;
  if (plan.filesToCreate?.length) result += `# Files To Create\n${plan.filesToCreate.map(f => `- ${f}`).join('\n')}\n\n`;
