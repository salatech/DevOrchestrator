...
describe('workspace/manager', () => {
  describe('findProjectRoot', () => {
    it('returns the dir if package.json exists', async () => {
      vi.spyOn(filesystem, 'fileExists').mockImplementation(async (p: string) =>
    ...
    it('traverses up until it finds a marker', async () => {
      vi.spyOn(filesystem, 'fileExists').mockImplementation(
        async (p: string) => p === path.resolve('/mock/dir/package.json'),
      );
      vi.spyOn(filesystem, 'pathExists').mockImplementation(async (p: string) => false);
